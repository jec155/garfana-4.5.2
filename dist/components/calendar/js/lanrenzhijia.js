...
System.register([], function (_export, _context) {
	...
	function f_tcalParseDate(s_date, s_format) {
		...
		for (var n = 0; n < s_format.length; n++) {
			...
			} else if (s_char == ' ') s_regexp += '\\s';else s_regexp += (s_char.match(/[\w\d]/) ? '' : '\\') + s_char;
		}
	...
	return {
		...
		execute: function () {
			...
			{ 't': 'Y', 'r': '19\\d{2}|20\\d{2}', 'p': function p(d_date, n_value) {
					d_date.setFullYear(Number(n_value));return d_date;
				}, 'g': function g(d_date) {
					var n_year = d_date.getFullYear();return n_year;
				} },
			// Numeric representation of a month, with leading zeros
			{ 't': 'm', 'r': '0?[1-9]|1[0-2]', 'p': function p(d_date, n_value) {
					d_date.setMonth(Number(n_value) - 1);return d_date;
				}, 'g': function g(d_date) {
					var n_month = d_date.getMonth() + 1;return (n_month < 10 ? '0' : '') + n_month;
				} },
			// A full textual representation of a month, such as January or March
			{ 't': 'F', 'r': A_TCALCONF.months.join('|'), 'p': function p(d_date, s_value) {
					for (var m = 0; m < 12; m++) {
						if (A_TCALCONF.months[m] == s_value) {
							d_date.setMonth(m);return d_date;
						}
					}
				}, 'g': function g(d_date) {
			...
			{ 't': 'd', 'r': '0?[1-9]|[12][0-9]|3[01]', 'p': function p(d_date, n_value) {
					d_date.setDate(Number(n_value));if (d_date.getDate() != n_value) d_date.setDate(0);return d_date;
				}, 'g': function g(d_date) {
					var n_date = d_date.getDate();return (n_date < 10 ? '0' : '') + n_date;
				} },
			// Day of the month without leading zeros
			{ 't': 'j', 'r': '0?[1-9]|[12][0-9]|3[01]', 'p': function p(d_date, n_value) {
					d_date.setDate(Number(n_value));if (d_date.getDate() != n_value) d_date.setDate(0);return d_date;
				}, 'g': function g(d_date) {
					var n_date = d_date.getDate();return n_date;
				} },
			// A full textual representation of the day of the week
			{ 't': 'l', 'r': A_TCALCONF.longwdays.join('|'), 'p': function p(d_date, s_value) {
					return d_date;
				}, 'g': function g(d_date) {
					return A_TCALCONF.longwdays[d_date.getDay()];
				} },
			// English ordinal suffix for the day of the month, 2 characters
			{ 't': 'S', 'r': 'st|nd|rd|th', 'p': function p(d_date, s_value) {
					return d_date;
				}, 'g': function g(d_date) {
					n_date = d_date.getDate();if (n_date % 10 == 1 && n_date != 11) return 'st';if (n_date % 10 == 2 && n_date != 12) return 'nd';if (n_date % 10 == 3 && n_date != 13) return 'rd';return 'th';
				} }];
...
//# sourceMappingURL=lanrenzhijia.js.map
